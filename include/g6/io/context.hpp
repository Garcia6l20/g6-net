...
namespace std {
    std::exception_ptr tag_invoke(unifex::tag_t<unifex::get_exception_ptr>, std::error_code &&ec) noexcept {
        return std::make_exception_ptr(std::system_error{std::forward<std::error_code>(ec)});
    }
}
...
namespace g6::io {
    namespace detail {
        ...
        consteval auto io_uring_op_code() noexcept {
            constexpr uint8_t io_uring_ops[] = {IORING_OP_READV,   IORING_OP_WRITEV,  IORING_OP_SEND,
                                                IORING_OP_RECV,    IORING_OP_SENDMSG, IORING_OP_RECVMSG,
                                                IORING_OP_CONNECT, IORING_OP_ACCEPT};
    ...
    struct context::completion_base : public context::underlying_context::completion_base {
        using context::underlying_context::completion_base::completion_base;
    };
    ...
    public:
        ...
        explicit base_operation(const auto &sender, Receiver2 &&r)
            : context_(sender.context_), fd_(sender.fd_), receiver_((Receiver2 &&) r) {}
        template<typename Receiver2>
        explicit base_operation(io::context &ctx, int fd, Receiver2 &&r)
            : context_(ctx), fd_(fd), receiver_((Receiver2 &&) r) {}
    ...
    private:
        ...
        static void on_schedule_complete(operation_base *op) noexcept { static_cast<base_operation *>(op)->start_io(); }
        ...
        void start_io() noexcept {
            ...
            auto populateSqe = [this](io_uring_sqe &sqe) noexcept {
                ...
                sqe.user_data = reinterpret_cast<std::uintptr_t>(static_cast<base_operation *>(this));
            ...
            auto oldState = state_.fetch_add(io_epoll_context::read_sender::operation<Receiver>::io_flag,
            ...
            if (result == -ECANCELED) {
                ...
                if constexpr (is_nothrow_receiver_of_v<Receiver, ssize_t>) {
                    ...
                    UNIFEX_TRY { unifex::set_value(std::move(receiver_), ssize_t(result)); }
                    UNIFEX_CATCH(...) { unifex::set_error(std::move(receiver_), std::current_exception()); }
                }
            } else {
                unifex::set_error(std::move(receiver_), std::error_code{-int(result), std::system_category()});
            }
#elif G6_IO_USE_IOCP_CONTEXT
#endif
        }
        ...
        static void on_operation_complete(operation_base *op) noexcept {
            ...
            if (self.result_ >= 0) {
                if constexpr (requires { {self.set_value(std::move(self.receiver_))}; }) {
                    self.set_value(std::move(self.receiver_));
                } else {
                    G6_IO_SET_VALUE(self.receiver_, size_t(self.result_));
                }

            } else if (self.result_ == -ECANCELED) {
                unifex::set_done(std::move(self.receiver_));
            } else {
                unifex::set_error(std::move(self.receiver_), std::error_code{-self.result_, std::system_category()});
            }
        }
