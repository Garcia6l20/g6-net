...
namespace g6::ssl {
    namespace detail {
        using mbedtls_x509_crt_ptr = g6::c_shared_ptr<mbedtls_x509_crt, mbedtls_x509_crt_init, mbedtls_x509_crt_free>;
    }
    class certificate final {
    public:
        certificate() noexcept = default;

        certificate(const certificate &other) { load(other.chain()); }

        certificate(certificate &&) = default;

        explicit certificate(std::string_view path) { load(path); }
        ...
        void load(std::span<T> data) {
            if (auto error = mbedtls_x509_crt_parse(crt_.get(), reinterpret_cast<const uint8_t *>(data.data()),
                                                    data.size_bytes());
                error != 0) {
                throw std::system_error{error, ssl::error_category, "mbedtls_x509_crt_parse"};
            }
        }
        ...
        [[nodiscard]] mbedtls_x509_crt &chain() { return *crt_; }
