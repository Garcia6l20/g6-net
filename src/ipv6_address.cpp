...
namespace { namespace local {
    ...
    constexpr std::uint8_t digit_value(char c) { return static_cast<std::uint8_t>(c - '0'); }
...
std::string g6::net::ipv6_address::to_string() const {
    ...
    for (std::uint32_t i = 0; i < 8;) {
        if (m_bytes[2 * i] == 0 && m_bytes[2 * i + 1] == 0) {
            ...
            ++i;
            while (i < 8 && m_bytes[2 * i] == 0 && m_bytes[2 * i + 1] == 0) { ++i; }
    ...
    auto appendPart = [&](std::uint32_t index) {
        ...
        if (highByte > 0 || lowByte > 15) {
            if (highByte > 0) {
                if (highByte > 15) { *c++ = local::hex_char(highByte >> 4); }
...
namespace g6::net {
    std::optional<g6::net::ipv6_address> tag_invoke(tag_t<g6::from_string<ipv6_address>>,
        ...
        while (pos < length && partCount < 8) {
            ...
            if (pos < length) {
                ...
                if ((pos + 1) == length) { return std::nullopt; }
                ...
                if (string[pos] == ':') {
                    ...
                    // with too many.
                    if (partCount > 6) { return std::nullopt; }

                    // Check for over-long or octal notation.
                    if (digitCount > 3 || (digitCount > 1 && digits[0] == 0)) { return std::nullopt; }

                    // Check that digits are valid decimal digits
                    if (digits[0] > 9 || (digitCount > 1 && digits[1] > 9) || (digitCount == 3 && digits[2] > 9)) {
                        return std::nullopt;
                    }
...

                    {
                        ...
                        if (decimalParts[0] > 255) { return std::nullopt; }
                    }

                    for (int decimalPart = 1; decimalPart < 4; ++decimalPart) {
                        if (string[pos] != '.') { return std::nullopt; }
                        ...
                        if (decimalParts[decimalPart] > 255) { return std::nullopt; }
        ...
        if (partCount < 8) {
            if (!doubleColonPos) { return std::nullopt; }
            ...
            for (int i = 0; i < postCount; ++i) { parts[7 - i] = parts[7 - zeroCount - i]; }

            // Fill gap with zeroes.
            for (int i = 0; i < zeroCount; ++i) { parts[preCount + i] = 0; }
